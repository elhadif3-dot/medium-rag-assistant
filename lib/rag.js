...
export async function retrieveContext(question) {
  ...
  const lexicalCandidates = getLexicalCandidates(question, RAG_CONFIG.top_k, {
...
function selectContextForQuestion(question, contexts) {
  ...
  const maxSelected = preciseQuestion ? 2 : recommendationQuestion ? 4 : Math.min(RAG_CONFIG.top_k, requestedCount);
