...
If the user asks for an exact number of titles or results from 1 to 3, return exactly that number when the context supports it.
If the user asks for more than 3 titles or results, cap the answer at 3 distinct articles and do not refuse solely because the requested list is larger than 3.
...
If one subquestion asks for more than 3 titles or results, cap that sub-answer at 3 distinct articles.
...
export async function answerQuestion(question, options = {}) {
  ...
  if (isMultiResultListing(question) && asksOnlyForTitles(question)) {
    const response = buildListingTitleResponse(question, context);
    return {
      response,
...
function asksOnlyForTitles(question) {
  return /\breturn\s+only\s+the\s+titles?\b/i.test(question) ||
    /\bonly\s+titles?\b/i.test(question);
}

function buildListingTitleResponse(question, context) {
  const requestedCount = requestedResultCount(question);
  const titles = [];
  const seen = new Set();
...
function isClearlyExternalQuestion(question) {
  const normalized = String(question || "").toLowerCase();
  return (
    /\b(weather|forecast)\b/.test(normalized) ||
    /\b(current|today|tomorrow|yesterday|right now)\b/.test(normalized) && /\b(use current|current information|live|latest)\b/.test(normalized) ||
    /\b(ignore|disregard)\b.*\b(context|medium|retrieved)\b/.test(normalized) ||
    /\b(use|using)\s+(your\s+own\s+knowledge|the\s+internet|external\s+knowledge)\b/.test(normalized) ||
    /\b(student id|private email|password|api key)\b/.test(normalized) ||
    /\bwho won the 2022 fifa world cup\b/.test(normalized) ||
    /\bcapital of france\b/.test(normalized)
  );
}
