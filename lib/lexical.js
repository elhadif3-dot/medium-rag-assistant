...
const cachedIndexesByUrl = new Map();
const cachedIndexPromisesByUrl = new Map();

export async function getLexicalCandidates(question, limit, options = {}) {
  const index = await loadLexicalIndex(options.siteUrl);
...
async function loadLexicalIndex(siteUrl = "") {
  const lexicalIndexUrl = getRemoteLexicalIndexUrl(siteUrl);
  if (lexicalIndexUrl) {
    if (cachedIndexesByUrl.has(lexicalIndexUrl)) {
      return cachedIndexesByUrl.get(lexicalIndexUrl);
    }
    if (!cachedIndexPromisesByUrl.has(lexicalIndexUrl)) {
      cachedIndexPromisesByUrl.set(lexicalIndexUrl, readLexicalIndex(lexicalIndexUrl));
    }

    const index = await cachedIndexPromisesByUrl.get(lexicalIndexUrl);
    cachedIndexesByUrl.set(lexicalIndexUrl, index);
    return index;
  }
...
async function readLexicalIndex(remoteUrl = "") {
  remoteUrl ||= getRemoteLexicalIndexUrl();
  if (remoteUrl) {
...
function getRemoteLexicalIndexUrl(siteUrl = "") {
  if (siteUrl) {
    return `${siteUrl.replace(/\/$/, "")}/lexical-index.json`;
  }
